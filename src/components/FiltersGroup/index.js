...
const FiltersGroup = props => {
  ...
  const renderSalaryRangesList = () => {
    ...
    const onChangeRange = salaryRangeId => {
      updateSalaryRangeId(salaryRangeId)

      // Make an HTTP GET request with the selected salary range ID using the native fetch API
      fetch(`/your-api-endpoint?minimum_package=${salaryRangeId}`)
        .then(response => {
          if (!response.ok) {
      ...
      return (
        <li className="filters-list-item" key={eachRange.salaryRangeId}>
          <input
            ...
            name="salary ranges"
            onChange={() => onChangeRange(eachRange.salaryRangeId)}
            checked={isChecked}
          />
